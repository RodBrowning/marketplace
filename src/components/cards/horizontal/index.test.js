...
describe("Horizontal card", () => {
  test("Complete", () => {
    ...
    const horizontalCardBrand = screen.getByRole("heading", {
      level: 6,
      name: "Brand",
    });
    expect(horizontalCardBrand).toBeInTheDocument();

    const horizontalCardTitle = screen.getByRole("heading", {
      level: 3,
      name: "T-Shirt",
    });
    expect(horizontalCardTitle).toBeInTheDocument();

    const horizontalCardPrice = screen.getByRole("heading", {
      level: 6,
      name: "Price £15.00",
    });
    expect(horizontalCardPrice).toBeInTheDocument();

    const horizontalCardShipping = screen.getByRole("button", {
      name: "Shop now",
    });
