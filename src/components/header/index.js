...
const Header = () => {
    ...
    useEffect(() => {
        ...
        if(cartList.length > 0){
            ...
            timeout = setTimeout(() => cartCountRef.current.classList.remove('animate'), 350);
        }
        return () => {
                    ...
                    <li>
                        <NavLink to={"cart"} end>
                            <span className='cart-btn'>
                                {cartList && cartList.length > 0 && <p data-testid="num-itens" className="num-itens" ref={cartCountRef}>{cartList.length < 100 ? cartList.length : '+99'}</p>}
