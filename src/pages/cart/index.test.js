...
describe("Cart page", () => {
  ...
  test("Render products in the table", async () => {
    ...
    const imgAlt = await screen.findByAltText(
      /Lorem ipsum dolor sit amet consectetur0./i
    );
  ...
  test("Total values", async () => {
    ...
    let totalToPay = await screen.findByRole("heading", {
      level: 5,
      name: /Total/i,
    });
  ...
  test("Total values after remove product from slice", async () => {
    ...
    expect(totalShipping).toBeInTheDocument();
    expect(totalShipping.innerHTML).toBe(
      '<span class="free-shipping">Free</span>'
    );

    totalToPay = await screen.findByRole("heading", {
      level: 5,
      name: /Total/i,
    });
  ...
  test("Not propagate event", async () => {
    ...
    let actionButtonsWrappers = await screen.findAllByTestId(
      "not-propagate-click-event"
    );
