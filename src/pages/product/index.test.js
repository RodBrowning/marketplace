...
describe("Product page", () => {
  test("Complete component", async () => {
    ...
    const productPageImageAlt = await screen.findByAltText(
      "Lorem ipsum dolor sit amet consectetur."
    );
    expect(productPageImageAlt).toBeInTheDocument();

    const productPageBrand = await screen.findByRole("heading", {
      level: 6,
      name: "brandA",
    });
    expect(productPageBrand).toBeInTheDocument();

    const productPageTitle = await screen.findByRole("heading", {
      level: 2,
      name: "titleA",
    });
    expect(productPageTitle).toBeInTheDocument();

    const productPagePrice = await screen.findByRole("heading", {
      level: 4,
      name: "£15.00 / £20.00",
    });
    ...
    const productPageDescription = await screen.findByText(
      /Lorem ipsum dolor sit amet consectetur/i
    );
    expect(productPageDescription).toBeInTheDocument();

    const productPageShipping = await screen.findByRole("heading", {
      level: 6,
      name: "Free Shipping",
    });
  ...
  test("Without free shipping", async () => {
    ...
    const productPageShipping = screen.queryByRole("heading", {
      level: 6,
      name: "Free Shipping",
    });
  ...
  test("In the cart", async () => {
    ...
    const productPageButtonElement = await screen.findByText(
      /Remove from cart/i
    );
    expect(productPageButtonElement).toBeInTheDocument();
  });
